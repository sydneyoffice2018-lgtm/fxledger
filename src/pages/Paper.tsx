/**
 * STEP 1: PAPER
 * Client brings paper → we record it → assign to supplier
 */
...
export function PaperPage() {
  ...
  // Only show orders at paper stage
  const paperOrders = (orders as any[]).filter(o => o.status === 'cash_received');
  const allPaper = (orders as any[]).filter(o => o.status !== 'cancelled');
  ...
  const createMut = useMutation({
    mutationFn: () => api.post('/orders', {
      ...
      clientRate: '0',
      bbName: form.bbName,
      bbDepositRef: form.bbRef,
      note: form.note,
    }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['orders'] }); setShowNew(false); toast('Paper intake recorded'); setForm({ customerId: '', currency: 'AUD', amount: '', bbName: '', bbRef: '', note: '' }); },
    ...
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ color: 'var(--text3)', fontSize: 13, marginTop: 4 }}>Client hands over paper → record it → pass to BB for deposit</p>
        </div>
      ...
      <div style={{ background: 'var(--surface)', border: `1px solid ${STEP_COLOR}30`, borderRadius: 12, padding: '14px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 14 }}>Client gives you paper</div>
          <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 2 }}>Record the intake here → hand to BB → BB deposits to your bank → move to Step 2</div>
        </div>
        <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
          <div style={{ fontSize: 22, fontWeight: 800, fontFamily: "'JetBrains Mono',monospace", color: STEP_COLOR }}>{paperOrders.length}</div>
          <div style={{ fontSize: 11, color: 'var(--text3)' }}>awaiting BB</div>
        </div>
      </div>
      ...
      {paperOrders.length === 0 ? (
        ...
        <Table headers={['Client', 'Amount', 'BB', 'Received', 'Actions']}>
          {paperOrders.map(o => (
            <TR key={o.id}>
      ...
      <Modal open={showNew} onClose={() => setShowNew(false)} title="Record Paper Intake">
        ...
        <Input label="BB Name" value={form.bbName} onChange={f('bbName')} placeholder="Who will collect this paper?" />
        <Input label="BB Reference" value={form.bbRef} onChange={f('bbRef')} placeholder="Receipt or reference number" />
        <Input label="Note" value={form.note} onChange={f('note')} placeholder="Optional notes" />
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <Btn variant="ghost" onClick={() => setShowNew(false)}>Cancel</Btn>
          <Btn loading={createMut.isPending} onClick={() => createMut.mutate()} disabled={!form.customerId || !form.amount} style={{ background: STEP_COLOR, color: '#0c0e12' }}>
            Record Intake
          </Btn>
        </div>
      </Modal>
